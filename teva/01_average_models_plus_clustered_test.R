...
run_experiment <- function(exp_number, ratio){
  ...
  cluster_sizes = table(pos_cluster_membership)
  ...
  for (c in clusters){
    ...
    cluster_neg_id = cluster_neg_data_info %>% 
      ...
      # top_n will include more than n if there are ties, not sure how to fix it
      select(patient_id, matched_patient_id)
    ...
    cd = create_dataset(cluster_data, "label", "patient_id", 
                        "matched_patient_id", paste("Teva-", c, sep=""))
  ...
  pr_curves[["recall"]] = pr_train2$curve$rec_binned
  ...
  for (c in clusters){
    ...
    pr_name = paste("clustered", c, "all", sep="_")
    pr_curves[[pr_name]] = pr$curve$prec[1:bin_num]
    
    # save all columns of pr curve
    pred_file = paste("clustered", c, "_all.csv", sep="")
  ...
  readr::write_csv(pr$curve, file.path(exp_folder, "clustered_avg.csv"))
  ...
  readr::write_csv(pr$curve, file.path(exp_folder, "clustered_weighted_avg.csv"))
  ...
  readr::write_csv(pr$curve, file.path(exp_folder, "clustered_closest.csv"))
  
  # ----------------------------------------------------------------------------
  # check which model did better than the benchmark
  ...
  better_than_random <- function(curves){
    ...
    as.data.frame(lapply(curves[,3:ncol(curves)], check_col))
  }
  ...
  # ----------------------------------------------------------------------------
  # John wants to see if we can do better than the benchmark model just for 
  # some clusters, so we'll generate a pr curve separately for each cluster
  # model and benchmark model in a given cluster
  
  pr_curves2 = list()
  for (c in clusters){
    c_int = as.integer(c)
    cluster_size_train = cluster_sizes[[c_int]]
    cluster_mask = closest_model == c_int
    cluster_size_test = sum(cluster_mask)
    
    # save cluster specific pr curve for benchmark model
    cluster_train2_pred = pred_train2
    cluster_train2_pred$data = cluster_train2_pred$data[cluster_mask,]
    
    # check if we have two classes left in this cluster - ROCR fails otherwise
    if(length(unique(cluster_train2_pred$data$response)) == 2){
      # even this wasn't enough for some datasets and ROCR failed, so let's do
      # a bit of defensive programming with R's try catch
      pr <- tryCatch({
        binned_perf_curve(cluster_train2_pred, x_metric="rec", 
                          y_metric="prec", bin_num=bin_num)
      }, error = function(e){
        warning(paste("Error in pr curve calculation for cluster", c, 
                      "in model", exp_name, sep=" "))
      })
      if(typeof(pr) != "character"){
        pr <- binned_perf_curve(cluster_train2_pred, x_metric="rec", 
                                y_metric="prec", bin_num=bin_num)
        pr_name = paste("train2_c", c, "tr", cluster_size_train, sep="_")
        pr_binned_train2 = pr$curve$prec[1:bin_num]
        pr_curves2[[pr_name]] = pr_binned_train2
        pr_file = paste(pr_name, ".csv", sep="")
        readr::write_csv(pr$curve, file.path(exp_folder, pr_file))
        
        # save cluster specific pr curve for cluster model
        pred_clustered = clustered_preds_obj[[c]]
        pred_clustered$data = pred_clustered$data[cluster_mask,]
        
        pr <- binned_perf_curve(pred_clustered, x_metric="rec", y_metric="prec", 
                                bin_num=bin_num)
        pr_name = paste("clustered", c, "te", cluster_size_test, sep="_")
        pr_binned_clustered = pr$curve$prec[1:bin_num]
        pr_curves2[[pr_name]] = pr_binned_clustered
        pr_file =  paste(pr_name, ".csv", sep="")
        readr::write_csv(pr$curve, file.path(exp_folder, pr_file))
        
        # check if clustered model is better on cluster x than benchmark
        better = as.integer(pr_binned_clustered > pr_binned_train2)
        num_better = num_better + sum(better[1:bin_num-1], na.rm = T)
        pr_curves2[[paste("better_", c, sep="")]] = better
      }
    }
  }
  ...
  final_pr = bind_cols(pr_curves, pr_curves_mask, pr_curves2)
...
for (e in 9:nrow(experiments)){
  run_experiment(e, ratio=10)  
}